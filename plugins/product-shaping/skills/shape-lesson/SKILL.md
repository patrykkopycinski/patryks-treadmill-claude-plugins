...
# /shape-lesson — Capture a Recurring Rule

Append a single entry to `context/foundation/lessons.md` so future agents (and future humans) can re-read it as a prior when planning, implementing, or reviewing related work. Invoke it inline when you notice a pattern worth surfacing.
...
1. **If a freeform description was provided inline** (e.g. `/shape-lesson feature flags should always have a kill date`), use it as a seed for the Rule field and proceed to the interview.
...
Before reading or writing anything under `context/`, apply the **context-root protocol** documented in `../shape-init/references/context-root-protocol.md`. It is idempotent — if `context/` is already excluded (or if you are not inside a git repo), it is a no-op. The protocol resolves `CONTEXT_ROOT` to the main working tree of the repo (so worktrees share one `lessons.md` file) and appends `context/` to the shared `<git-common-dir>/info/exclude` if needed. Treat `context/foundation/lessons.md` in the rest of this skill as `$CONTEXT_ROOT/context/foundation/lessons.md`. Surface a one-line `git-exclude: <state>` in this skill's final summary, plus a `context-root: <path>` line when `CONTEXT_ROOT` differs from cwd (so the user knows their lesson landed in the shared, worktree-spanning location).
...
- **Applies to** — comma-separated list of skill names or work-types this rule should weigh most for: `frame`, `research`, `plan`, `implement`, `review`. Use `all` if the rule cuts across the whole lifecycle.
...
If `context/foundation/lessons.md` does not exist, create it with this canonical 3-line header (embedded inline — no separate template file):

```
# Lessons Learned

> Append-only register of recurring rules and patterns. Re-read at the start of planning, implementation, and review work.
...
- **Self-bootstrap is the default.** Don't tell the user "run /shape-init first" — create the file with the canonical header on first use. (`/shape-init` creates the `/context` directory skeleton; this skill owns `lessons.md` end-to-end.)
- **Pre-fill nothing.** This skill expects the user to do the writing. That's the price of capturing rules outside a structured review.
