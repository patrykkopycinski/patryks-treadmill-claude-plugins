---
name: shape-e2e
description: Drive an approved plan's browser-level (E2E) phases against the running app, one risk at a time — plan → generate → review → verify. The E2E sibling of /shape-implement and /shape-tdd, sharing the same plan and Progress. Only drives risks that genuinely need a browser and whose feature is already built; redirects the rest to /shape-tdd or /shape-implement. Use when the user says "e2e", "write/generate a Playwright test", "browser test this risk", or "drive the plan's E2E phases".
model: opus
allowed-tools:
  - Read
  - Glob
...
This skill is the **E2E sibling of `/shape-implement` and `/shape-tdd`**. It reads the same plan, mutates the same canonical `## Progress` section, and uses the same phase-end commit ritual and clipboard handoffs. The difference is the inner loop: instead of writing production code (`/shape-implement`) or a failing unit test first (`/shape-tdd`), you generate and harden a browser-level test against the **running app**. Because the three skills share `## Progress`, you can interleave them freely — build a feature with `/shape-implement`, unit-test the next phase with `/shape-tdd`, then come back here to add the E2E layer for a cross-boundary risk, and state is never lost.
...
- **The feature under test already exists.** E2E runs against a real, running app — so unlike `/shape-tdd`, the implementation must be **present**, not absent. If the phase's feature isn't built yet, there is nothing for the browser to drive; stop and redirect to `/shape-implement` (or `/shape-tdd`) to build it first, then come back.
...
After all phases →  Completion summary + optional /shape-impl-review
```
...
1. **Resolve the plan**:
   - `/shape-e2e <change-id> [phase N]` → `context/changes/<change-id>/plan.md`.
...
1. A change-id (e.g., `/shape-e2e save-session phase 6`), or
...
This skill assumes Playwright is already installed; it won't set it up. Options:
  • Install and configure Playwright first (npm init playwright@latest), then re-run /shape-e2e.
  • Use /shape-tdd or /shape-implement for non-browser coverage.
```
...
A risk needs E2E when it **crosses several system boundaries** (auth, routing, API, DB) or **exists only in the rendered UI**. If an isolated function, endpoint contract, or integration test could prove the risk, E2E is the wrong (slow, brittle) tool — drive it with `/shape-tdd` or `/shape-implement` instead.

| E2E-worthy — drive it here | Not E2E-worthy — redirect to /shape-tdd or /shape-implement |
|---|---|
...
### Feature-presence check (the inverse of /shape-tdd)
...
Copy `/shape-implement <change-id> phase [N]` to the clipboard, display it with `(✓ copied)`, and STOP.
...
- If it's **mixed or ambiguous** (e.g., a phase that's partly an endpoint contract, partly a rendered-UI flow), use `AskUserQuestion`:
  ...
  - question: "Phase [N] mixes an isolated-function risk and a browser-level flow. How should I drive it?"
    ...
    - label: "E2E the browser-level part (Recommended)"
      description: "I'll plan→generate→review→verify the cross-boundary flow and redirect the isolated-function part to /shape-tdd."
    - label: "Redirect whole phase to /shape-tdd"
...
- question: "Phase [N] isn't a good E2E fit. How do you want to handle it?"
  ...
  options:
  - label: "Hand off to /shape-tdd (Recommended)"
    description: "Copy `/shape-tdd <change-id> phase N` to the clipboard. Clear context, run it, then resume E2E on the next phase."
  - label: "Hand off to /shape-implement"
    description: "Copy `/shape-implement <change-id> phase N` to the clipboard if test-first doesn't fit either."
...
→ /shape-tdd <change-id> phase [N] (✓ copied)

Clear context (`/clear`), run that, then come back with:
→ /shape-e2e <change-id> phase [N+1]
```
...
When all `#### Automated` rows in `### Phase N:` are `[x]`, run the phase-end ritual (this mirrors `/shape-implement` and `/shape-tdd` — one Conventional-Commits commit per phase, then write its short SHA back into the rows that flipped).
...
8. **Capture and write back the SHA.** `git rev-parse --short HEAD` → `SHA`. For every Progress row flipped this phase, Edit `- [x] N.M <title>` → `- [x] N.M <title> — <SHA>` (skip rows that already carry a SHA; if `SHA=""`, skip — `/shape-archive` surfaces SHA-less rows as informational warnings).
...
- question: "Phase [N] complete (E2E). How to proceed?"
  ...
  - label: "Review this phase first"
    description: "Run /shape-impl-review to verify the implementation against the plan before continuing."
  multiSelect: false

**Continue:** read the next phase, set its task `in_progress`, run the E2E gate, proceed. No need to re-read the whole plan.

**Review:** run `/shape-impl-review @<path-to-plan> phase [N]`, then re-present the continue/clear decision (without the review option).

**Clear:** copy `/shape-e2e <change-id> phase [N+1]` to the clipboard (per the clipboard convention) and display it as `→ /shape-e2e <change-id> phase [N+1] (✓ copied)`.
...
**The `## Progress` section in `plan.md` is the single source of truth** — no state file, no comment markers. This skill mutates Progress exactly like `/shape-implement` and `/shape-tdd`: flip `[ ]` → `[x]` per step as it lands; append the closing commit's SHA to every row that flipped, in one shot at phase end. Mid-phase, completed rows sit `[x]` without a SHA — a valid intermediate state. Because all three skills write the same section identically, a change can be driven by any of them, in any order.
...
2. **Update `change.md`**: `status: implemented`, `updated: <today>`. (Do NOT set `archived_at` — that's `/shape-archive`.)
...
Summary:
- Phases completed: [N]  ([k] E2E'd, [j] redirected to /shape-tdd or /shape-implement)
- E2E tests added: [count] across [files], each tied to a test-plan.md risk
- Levers in place: seed.spec.ts + E2E rules
```

   Then `AskUserQuestion`: run `/shape-impl-review <change-id>` (full-plan review) or skip.
