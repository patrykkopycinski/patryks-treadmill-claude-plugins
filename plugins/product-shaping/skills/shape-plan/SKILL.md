 ...
 **Example AskUserQuestion call with recommendations (non-software — content/strategy):** `Depth` is `[D]` — diagnostic about audience/scope; skip if a frame brief already settled who this is for.
 ...
 description: "Concepts paired with step-by-step exercises. · Strength: Balances understanding and practice — matches the format that got highest completion rates in 2. · Tradeoff: 2-3x more prep time per lesson; requires working example repos."
...
1. **Resolve the change folder, then write the plan** to `context/changes/<change-id>/plan.md`.
 - If the user invoked `/shape-plan <change-id>` and `context/changes/<change-id>/` already exists, use it.
...
- **If context feels degraded during planning** — if responses become sluggish or repetitive, save the current plan draft to file and offer the user to continue in a fresh context:
 ...
 ```
 This lets `/shape-plan` reload the draft and continue iterating with full context available.
...
- question: "What should the learner be able to DO after this module — not just know?"
 ...
 description: "Learner produces a functional artifact using the techniques taught. · Strength: Forces genuine skill transfer — the artifact proves competence. Matches the 'Innovate' lesson format from 3. · Tradeoff: Requires well-designed starter templates and clear acceptance criteria; takes 2-3x longer to prep."
...
- question: "How should this module handle different skill levels in the audience?"
 ...
 description: "Two parallel paths diverging early. · Strength: Each audience gets perfectly targeted content. · Tradeoff: 2x production cost; splitting a small group may hurt community dynamics."
 multiSelect: false
