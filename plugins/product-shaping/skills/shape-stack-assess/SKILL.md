...
This skill is the brownfield counterpart to `/shape-tech-stack`. Where tech-stack-selector helps greenfield users **choose** a stack, stack-assess helps brownfield users **evaluate** theirs. It reuses the same four agent-friendly quality gates (`references/agent-friendly-criteria.md`) but applies them as an evaluation lens rather than a selection filter.

The skill sits in the brownfield chain: `/shape-idea → /shape-prd → /shape-stack-assess → /shape-health-check`. Its single job: evaluate the existing stack against the quality gates and produce a structured assessment with concrete compensation strategies.
...
**Skip when**: the user is starting a new project from scratch — redirect to `/shape-tech-stack`. Skip also when the user just wants a dependency audit or security scan without the quality-gate framing — that is `/shape-health-check` territory.

## Relationship to other skills

- `/shape-idea` — upstream. Produces `shape-notes.md` with `context_type: brownfield`.
- `/shape-prd` — upstream. Produces `context/foundation/prd.md` (brownfield template). Optional input — the skill can run without a PRD.
- `/shape-tech-stack` — greenfield parallel. Same quality gates, different job (selection vs evaluation).
- `/shape-health-check` — downstream consumer. Reads `context/foundation/stack-assessment.md` to focus health checks on identified gaps.
...
1. **If a path argument was provided** (e.g. `/shape-stack-assess @context/foundation/prd.md`), strip a leading `@` if present and use the path as the PRD location for this run. The PRD is optional context, not a precondition — the skill runs without it.
...
```
No project markers found in the current directory. /shape-stack-assess requires an existing codebase.
If you're starting from scratch, use /shape-tech-stack instead.
```
...
```bash
test -f "$CONTEXT_ROOT/context/foundation/stack-assessment.md"
```
...
<overall verdict, key strengths, key gaps, and recommended next step (/shape-health-check)>
```
...
```bash
echo -n "/shape-health-check" | pbcopy 2>/dev/null || echo -n "/shape-health-check" | clip.exe 2>/dev/null || echo -n "/shape-health-check" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/shape-health-check"
```
...
═══════════════════════════════════════════════════════════
  ...
  ► Next:        /shape-health-check  (✓ copied to clipboard)
═══════════════════════════════════════════════════════════
```

STOP. Do not chain into `/shape-health-check` automatically — the user runs it when ready.
