...
**Skip when**: the PRD is hollow (large `## Open Questions`, `# TODO: domain rule`) — point at `/shape-prd` (or upstream `/shape-idea`) first; a roadmap from a hollow PRD will inherit the hollowness. Also skip when the user wants to plan a *single* change in detail — that's `/shape-plan`. The roadmap is plural; the plan is singular.

## Relationship to other skills

- `/shape-idea` and `/shape-prd` — produce the upstream PRD this skill consumes. If `shape-notes.md` carries a `## Forward: technical-roadmap` block (where shape parks roadmap-bound content), this skill lifts it.
- `shape-tech-stack` — runs between `/shape-prd` and this skill in the bootstrap chain. If `context/foundation/tech-stack.md` exists, this skill reads it as input to derive `## Foundations` (auth scaffold, deploy skeleton, observability — anything the tech-stack-selection step implied) and to short-circuit baseline probes for layers already declared.
