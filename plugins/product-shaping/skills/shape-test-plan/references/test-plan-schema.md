...
This is the canonical structure for the test plan produced by `/shape-test-plan`. The file is **both a phased rollout strategy and a growing cookbook**:
...
- **Cookbook** (§6) starts as placeholders and **fills in incrementally** as rollout phases ship. Each plan's final sub-phase (per the `/shape-plan` constraint baked in by the orchestrator) updates the cookbook with patterns the rollout phase just delivered.
...
>
> Refresh: re-run `/shape-test-plan --refresh` when stale (see §8).
>
...
3. **Risks are scenarios, not code locations.** This plan documents *what
   ...
   produced by `/shape-research` during each rollout phase. If the plan and
...
Short by design. Tone is imperative — these are the rules every downstream phase obeys. Principle #3 is the load-bearing distinction between this plan (QA spec) and `/shape-research` (knowledge extraction).
...
`/shape-research` during each rollout phase. If the plan tries to cite them
...
is the durable handoff from risk analysis to `/shape-research` and
`/shape-plan`.

```markdown
### Risk Response Guidance

| Risk | What would prove protection | Must challenge | Context `/shape-research` must ground | Likely cheapest layer | Anti-pattern to avoid |
|------|-----------------------------|----------------|--------------------------------------|-----------------------|-----------------------|
...
via `/shape-new`. Status moves left-to-right through the values below; the
...
those belong in per-phase `/shape-research`.
```
...
**This is the section that fills in over time.** The orchestrator writes placeholders during Phase 4; each rollout phase's plan updates the relevant sub-section in its final sub-phase (per the `/shape-plan` constraint).
...
(Optional. After each phase lands, /shape-implement appends a 2-3 line note
...
Refresh (`/shape-test-plan --refresh`) when:
...
- **No motivational phrasing** ("write better tests", "be a rockstar engineer").
- **No emojis.**
...
## How `/shape-test-plan` maintains this file
...
- **After each rollout phase completes** (via `/shape-implement`'s final sub-phase, baked into the plan by the orchestrator's planning prompt): the relevant §6 sub-section is filled in; §8 dates may bump if tools were verified.
