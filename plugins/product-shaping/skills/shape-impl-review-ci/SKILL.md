...
description: >
  ...
  GitHub Actions, GHA, Codex Action, automated PR review, or "review
  this PR in CI".
...
Run inside GitHub Actions (typically via Codex Action) on a pull request. Analyze the PR's changes against the plan it claims to implement. Emit an audit trail as a committed report file plus a concise PR comment.
...
git config user.email "41898282+Codex[bot]@users.noreply.github.com"
...
**Use the MCP tool, not `gh api`.** Codex-action v1 exposes `mcp__github_inline_comment__create_inline_comment` via its built-in MCP server. It wraps the per-comment endpoint (`POST /pulls/:n/comments`), which errors **loudly** on bad line positions — unlike the batched review endpoint, which silently drops invalid entries and leaves you with an empty review shell. Post each finding as its own tool call, with `confirmed: true` so the action's Haiku classifier doesn't buffer-then-filter (appropriate for a deterministic CI reviewer — every finding we emit is already the triage decision).
...
Why `confirmed: true`: the action's default is to buffer unconfirmed calls, run them through a Haiku classifier ("is this a real review or a test?"), and post only the reals in a post-step. That's useful for conversational review flows where Codex is exploring. For this skill every finding is already a triage decision — we want them posted as-is, synchronously, so the workflow's verdict-check step sees the final state.
...
# after Codex-action reads the verdict from the committed report
...
Mark the CI review task `completed`. You're done — the workflow's verdict-check step (after Codex-action) reads the committed report's verdict and fails the check when `REJECTED`, respecting the `impl-review-override` label.
