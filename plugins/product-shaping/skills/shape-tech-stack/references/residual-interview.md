...
1. If PRD body has a `## Forward: tech-stack` block (carried forward from `/shape-idea`) and that block names a language preference, use it.
...
Ask the user: "Project name for the hand-off (will be the directory name `/shape-bootstrap` scaffolds)?" with these options:
