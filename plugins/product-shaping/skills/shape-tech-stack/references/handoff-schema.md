...
`context/foundation/tech-stack.md` is the file `/shape-tech-stack` writes and `/shape-bootstrap` reads. This doc is the contract for its shape. Both skills load this file by relative path; renames or restructurings here are load-bearing.
...
The key from `references/starter-registry.yaml` `starters:` map. The validator (`a registry-sync validator (not bundled in this plugin)`) ensures bootstrapper only references keys present in this registry; tech-stack-selector is the source of truth.

Examples: `next`, `t3`, `fastapi`, `django`, `rails`, `spring`, `laravel`, `go`, `rust`, `expo`, `flutter`, `dotnet`.
...
Kebab-case. Drawn from PRD's `project` field unless the user overrode it during the project-name confirmation step. This is the directory name `/shape-bootstrap` will scaffold.
...
A solo developer shipping a recipe-matching MVP in 1 week with auth and an LLM
