...
description: >
  ...
  context/foundation/tech-stack.md hand-off /shape-bootstrap consumes. Use when
  ...
  scaffold. Use AFTER /shape-prd, BEFORE /shape-bootstrap.
...
This skill is the third link in the bootstrap chain (`/shape-idea → /shape-prd → shape-tech-stack → /shape-bootstrap`). Its single job: turn a written PRD into a recommended starter and a small machine-readable hand-off `/shape-bootstrap` can read to scaffold a project.
...
The starter registry in `references/starter-registry.yaml` is the **single source of truth** for available starters. `/shape-bootstrap` reads it; a CI validator (`a registry-sync validator (not bundled in this plugin)`) prevents bootstrapper from referencing a `starter_id` that does not exist here.
...
**Skip when**: `context/foundation/prd.md` is absent — the skill refuses and redirects to `/shape-idea` + `/shape-prd`. Skip also when the user is mid-implementation on an existing codebase asking about adding a library or replacing a single dependency — that is a per-change concern, not stack selection.

## Relationship to other skills

- `/shape-idea` — produces `shape-notes.md`, the precursor to PRD. Two upstream of this skill.
- `/shape-prd` — produces `context/foundation/prd.md`, the canonical input. Always upstream.
- `/shape-bootstrap` — downstream consumer. Reads `context/foundation/tech-stack.md` frontmatter and the registry; scaffolds the project.

## Required inputs

1. A PRD file — exists, is readable, conforms to the PRD schema (`skills/shape-idea/references/prd-schema.md`). Default location: `context/foundation/prd.md`. The user MAY pass a different path as the argument (see "Initial Response" below). The skill reads the **frontmatter** as priors (`product_type`, `target_scale`, `timeline_budget`, `project`) and may read body sections (`## Functional Requirements`, `## Non-Goals`) for the feature audit and to detect Socratic moments where the PRD's FRs surface a feature the recommended starter does not include.
...
1. **If a path argument was provided** (e.g. `/shape-tech-stack @context/foundation/prd-v2.md` or `/shape-tech-stack path/to/prd.md`), strip a leading `@` if present and use the path verbatim as the PRD location for this run.
...
```bash
echo -n "/shape-idea" | pbcopy 2>/dev/null || echo -n "/shape-idea" | clip.exe 2>/dev/null || echo -n "/shape-idea" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/shape-idea"
```

Print verbatim (substitute the resolved path; if defaulted, this is `context/foundation/prd.md`):

```
Tech-stack-selector requires a PRD at `<prd-path>`. Run `/shape-idea` first, then re-invoke.
```
...
- question: "Are these priors correct, or do you want to correct anything before we proceed?"
  ...
  - label: "Stop — fix the PRD first"
    description: "Exit. Re-run /shape-prd to fix priors, then re-invoke /shape-tech-stack."
  multiSelect: false
...
```bash
test -f "$CONTEXT_ROOT/context/foundation/tech-stack.md"
```
...
```bash
echo -n "/shape-bootstrap" | pbcopy 2>/dev/null || echo -n "/shape-bootstrap" | clip.exe 2>/dev/null || echo -n "/shape-bootstrap" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/shape-bootstrap"
```
...
═══════════════════════════════════════════════════════════
  ...
  ► Next:      /shape-bootstrap  (✓ copied to clipboard)
═══════════════════════════════════════════════════════════
```

STOP. Do not chain into `/shape-bootstrap` automatically — the user runs it when ready.
...
6. **Universal language only.** No private vault paths or organization-specific branding in shipped content. The recommended-defaults registry is multi-language by design; no single starter is "the" recommended path.
