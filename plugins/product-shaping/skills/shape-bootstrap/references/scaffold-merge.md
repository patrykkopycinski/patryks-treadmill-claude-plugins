...
Used by starters whose `cmd_template` starts with `git clone` (i.e., the starter is a public repo cloned and detached from its history).
...
Future starters get explicit overrides in v2 by adding entries to `bootstrapper-config.yaml`. The validator at `a registry-sync validator (not bundled in this plugin)` ensures every `starter_id` listed there exists in the tech-stack-selector registry.
...
6. Print verbatim (substitute `<exit_code>` and `<short_stderr>`):
   ...
   A partial verification log was written to `context/changes/bootstrap-verification/verification.md` with `phase_3_status: failed` and the captured CLI output. Re-invoke `/shape-bootstrap` (clipboard set) once the failure cause is addressed.
   ```
...
- **`{name}` appears more than once in `cmd_template`**: substitute every occurrence. Multi-occurrence example: `git clone <url> {name} && cd {name} && {pm} install` substitutes `{name}` twice.
...
- Does not generate `AGENTS.md` / `CLAUDE.md`. That is `/shape-agents-md`'s job.
