...
Once dependencies are installed, run the ecosystem's audit tool against the scaffolded tree. Surface the count and severity in conversation. Stage the full breakdown into the verification log. On day one of a fresh project, the user sees what kind of advisory state their dependency tree is shipping with — and the difference between direct and transitive findings.
...
- Does not generate `AGENTS.md` / `CLAUDE.md` based on findings. Surfacing audit findings into agent context is out of scope for this skill; users can fold them into AGENTS.md by hand if needed.
