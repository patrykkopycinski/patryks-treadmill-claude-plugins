...
This section is the audit trail completeness contract: a human reader (or `/shape-agents-md`) can see exactly which hints were carried forward into v1's run without bootstrapper acting on them. It is also where `quality_override: true` and `bootstrapper_confidence: best-effort` surface for the user — v1 surfaces but does not compensate.
